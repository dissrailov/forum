...
func (s *Sqlite) GetPostByCategory(categoryID int) ([]models.Post, error) {
	...
	rows, err := s.DB.Query(`
	SELECT p.id, p.title, p.content, p.image_url, p.likes, p.dislikes, p.user_id, p.created
	FROM posts p
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.ID, &post.Title, &post.Content, &post.ImageURL, &post.Likes, &post.Dislikes, &post.UserID, &post.Created)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", op, err)
		}
